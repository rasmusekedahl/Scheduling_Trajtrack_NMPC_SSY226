...
import os
import shutil
#from pkg_scheduler.sp_comsat.instance_maker import make_an_instance
...
class SchedulerConfig:
    def __init__(self, nodes=0, vehicles=0, tasks=0, edge_reduction=0, time_horizon=0, seed=0, dir='./output'):
        ...
        self.scheduler_result_filename = 'scheduler_result.json'
...
class Scheduler:
    ...
    def _parse_graph(self, input_file_path):
        ...
        out_graph = {"node_dict": {}, "edge_list": [], "start_list": {}, "charging_stations": []}

        # Parsing logic
        for _, node in enumerate(nodes):
            key = "d_{}".format(node)
            out_graph["node_dict"][key] = [nodes[node]["x"], nodes[node]["y"]]
        ...
        for _, edge in enumerate(edges):
            ...
            n1 = "d_{}".format(n[0])
            n2 = "d_{}".format(n[1])
            out_graph["edge_list"].append([n1, n2])
    ...
    def _problem_filename_to_config(self, problem_filename:str) -> SchedulerConfig:
        ...
        config = SchedulerConfig(nodes, vehicles, tasks, edge_reduction, time_horizon, seed, dir=self.config.output_folder)
        return config
    ...
    def schedule(self):
        ...
        instance, optimum, running_time, len_previous_routes, paths_changed, [node_sequence, current_routes] = \
            Compo_slim(problem=self.config.problem_filename, dir=self.config.output_folder)
        if optimum != 'None':
            make_Sequence_Planner_json(node_sequence, current_routes, dir=self.config.output_folder)

        # Open the file and load the JSON data
        with open(f'{self.config.output_folder}/{self.config.scheduler_result_filename}', 'r') as file:
            json_data = json.load(file)
        
        # parse it as per MPC requirement and convert it to csv
...
def example_single_problem():
    # config = SchedulerConfig(nodes=8, vehicles=2, tasks=6, edge_reduction=1.0, time_horizon=7000, seed=7)
    # config = SchedulerConfig(nodes=4, vehicles=1, tasks=1, edge_reduction=1.0, time_horizon=7000, seed=7)
    # config = SchedulerConfig(nodes=10, vehicles=4, tasks=4, edge_reduction=1.0, time_horizon=7000, seed=150)
    config = SchedulerConfig(nodes=16, vehicles=4, tasks=4, edge_reduction=1.0, time_horizon=7000, seed=150)
...
def example_multiple_problem():
    ...
    predefined_problems = ['MM_10_4_4_1.0_7000_150.json', 'MM_16_4_4_1.0_7000_150.json']

    for problem_filename in predefined_problems:
        # delete old files from output folder (except for directories)
        file_list = os.listdir(f'{scheduler.config.output_folder}/')
        for file in file_list:
            file_path = f'{scheduler.config.output_folder}/{file}'
